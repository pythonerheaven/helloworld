...
def _example_stock_quote(quote_ctx):
    ...
    """
    stock_code_list = ["US.BABA", "HK.00700"]
...
def _example_rt_ticker(quote_ctx):
    ...
    """
    stock_code_list = ["HK.00700", "US.BABA"]
...
def _example_get_trade_days(quote_ctx):
    ...
    获取交易日列表，输出 交易日列表
    """
    ret_status, ret_data = quote_ctx.get_trading_days("US", "2017-01-01", "2017-11-18")
    if ret_status == RET_ERROR:
        print(ret_data)
        exit()
    print("TRADING DAYS")
...
def _example_stock_basic(quote_ctx):
    ...
    """
    ret_status, ret_data = quote_ctx.get_stock_basicinfo("US", "IDX")
    if ret_status == RET_ERROR:
        print(ret_data)
        exit()
    print("stock_basic")
...
def _example_plate_subplate(quote_ctx):
    ...
    """
    ret_status, ret_data = quote_ctx.get_plate_list("US", "ALL")
    if ret_status == RET_ERROR:
        print(ret_data)
        exit()
...
if __name__ == "__main__":
    ...
    # 获取推送数据
    # quote_context.subscribe('US.BABA', "QUOTE", push=True)
    # quote_context.set_handler(StockQuoteTest())
    #
    #quote_context.subscribe('US.BABA', "K_1M", push=True)
    #quote_context.set_handler(CurKlineTest())
    #
    #
    # # today = datetime.today()
    # # pre_day = (today - timedelta(days=100)).strftime('%Y-%m-%d')
    # # _, prices = quote_context.get_history_kline('US.BABA', start=pre_day)
    #
    # quote_context.subscribe('HK.00700', "ORDER_BOOK", push=True)
    # quote_context.set_handler(OrderBookTest())
    #
    # quote_context.subscribe('US.BABA', "TICKER", push=True)
    # quote_context.set_handler(TickerTest())
    #
    # quote_context.subscribe('US.BABA', "RT_DATA", push=True)
    # quote_context.set_handler(RTDataTest())
    #
    # quote_context.subscribe('US.00700', "BROKER", push=True)
